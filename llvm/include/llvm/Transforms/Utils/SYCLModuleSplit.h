//===--------- SYCLModuleSplitter.h - split a module into callgraphs -------===//
//
...
// TODO(maksimsab):
//  * remove EntryPointGroup::Properties and maybe EntryPointGroup at all.
//  * remove virtual function's support until it gets upstreamed.
//  * perhaps, ModuleDesc::IsTopLevel worth to be removed as well as #ifdef NDEBUG
//  * maybe, SYCLDeviceRequirements should be upstreamed later.
//  * Stub string values for ModuleDesc::Name don't look OK.
//  * check GenXSPIRVWriterAdaptor comments
//  * fixupLinkageOfDirectInvokeSimdTargets and restoreLinkageOfDirectInvokeSimdTargets most likely should be removed.
//  * virtual functions are controversial but it might be OK for now.
//  * consider to remove verifyNoCrossModuleDeviceGlobalUsage
//  * Maybe fix doxygen comments.
//  * check isImportedFunction()'s history
...
struct EntryPointGroup {
  ...
  //SyclEsimdSplitStatus HasESIMD = SyclEsimdSplitStatus::SYCL_AND_ESIMD;
    ...
    Properties merge(const Properties &Other) const {
      ...
      // TODO(maksimsab): remove Properties from here.
      return Res;
    }
    ...
    // TODO(maksimsab): remove until later
...
Expected<std::vector<SYCLSplitModule>> parseSYCLSplitModulesFromFile(StringRef File);
