...
void dumpSplitModulesAsTable(
    const std::vector<SYCLSplitModule> &SplitModules,
    StringRef Path) {
  std::vector<std::string> Columns = {"Code"};
  SYCLStringTable Table;
  Table.emplace_back(std::move(Columns));
  for (const auto &SM : SplitModules) {
    std::vector<std::string> Row = {SM.ModuleFilePath};
...
Error runSYCLSplitModule(std::unique_ptr<Module> M) {
  ...
  auto SplitModulesOrErr =
    splitSYCLModule(std::move(M), Settings);
