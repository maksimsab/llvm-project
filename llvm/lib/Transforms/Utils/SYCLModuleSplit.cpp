...
ModuleDesc extractCallGraph(const ModuleDesc &MD,
  ...
  // TODO: remove or clarify this comment.
...
static Expected<SYCLSplitModule> saveModuleDesc(ModuleDesc &MD, std::string Prefix,
                                                bool OutputAssembly) {
  SYCLSplitModule SM;
...
Expected<std::vector<SYCLSplitModule>> parseSYCLSplitModulesFromFile(StringRef File) {
  ...
  static constexpr int NUMBER_COLUMNS = 2; // "Code" and "Symbols" at the moment.
  ++LI;
